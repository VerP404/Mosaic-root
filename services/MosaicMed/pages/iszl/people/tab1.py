...
from pages.iszl.people.query import sql_query_people_iszl, sql_query_people_iszl_all

type_page = "people-iszl"
alert_text = """Половозрастная структура прикрепленного населения по корпусам из ИСЗЛ
"""

tab1_layout_people_iszl = html.Div([
...
def data_processing(n_clicks, report_type):
    ...
    if triggered_id == f'get-data-button-{type_page}':
        ...
        if report_type == 'БУЗ ВО \'ВГП №3\'':
            bind_params = ''
            columns, data = TableUpdater.query_to_df(engine, sql_query_people_iszl_all, bind_params)
        else:
            bind_params = {
                'korp': report_type,
            }
            columns, data = TableUpdater.query_to_df(engine, sql_query_people_iszl, bind_params)

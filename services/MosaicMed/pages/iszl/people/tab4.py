...
from pages.iszl.people.query import sql_query_people_iszl, sql_query_people_iszl_all, sql_query_people_iszl_all_168n, \
    sql_query_people_iszl_168n

type_page = "people-iszl-168n"
alert_text = """Половозрастная структура прикрепленного населения по корпусам из ИСЗЛ находящихся на диспансреном наблюдении по приказу 168н
"""

tab4_layout_people_iszl = html.Div([
...
def data_processing(n_clicks, report_type):
    ...
    if triggered_id == f'get-data-button-{type_page}':
        ...
        if report_type == 'БУЗ ВО \'ВГП №3\'':
            bind_params = ''
            columns, data = TableUpdater.query_to_df(engine, sql_query_people_iszl_all_168n, bind_params)
        else:
            bind_params = {
                'korp': report_type,
            }
            columns, data = TableUpdater.query_to_df(engine, sql_query_people_iszl_168n, bind_params)

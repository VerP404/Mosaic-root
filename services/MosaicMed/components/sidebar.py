...
def get_sidebar():
    ...
    if current_user.is_authenticated:
        ...
        if 'other-reports' in modules:
            nav_items.append(dbc.NavLink([html.I(className="fas fa-chart-bar"), html.Span(" Другие отчеты", className="nav-text", title="Другие отчеты")], href="/other-reports", active="exact", className="nav-item", id="nav-other-reports"))
        if 'errors' in modules:
            nav_items.append(dbc.NavLink([html.I(className="fas fa-times-circle"), html.Span(" Отказы в оплате", className="nav-text", title="Отказы в оплате")], href="/errors", active="exact", className="nav-item", id="nav-errors"))
        if 'eln' in modules:
            nav_items.append(dbc.NavLink([html.I(className="fas fa-file-medical"), html.Span(" ЭЛН", className="nav-text", title="ЭЛН")], href="/eln", active="exact", className="nav-item", id="nav-eln"))
